...
if __name__ == '__main__':
    ...
    #写点到文件
    ff = open('/Users/makao/Yun/Workspace/lab/data/visual/num_nodes_gephi.csv','w')
    ...
    for item in user_final_intr_list:
        ...
        for i in range(0,len(user_interests_tags_list)):
            if i == len(user_interests_tags_list)-1:
                ...
                tag_color_str = tag_color_str + str(tag_color_number_list[int(user_interests_tags_list[i])]) + ','
        print >> ff, '%s,%s,"%s"' % (user_id,user_label,tag_color_str)
    
    ff.close()
    
    #写边到文件
    fff = open('/Users/makao/Yun/Workspace/lab/data/visual/num_edges_gephi.csv','w')
    ...
    for item in num_user_relation_list:
        ...
        for flwr in follower_list:
            print >> fff, '%s,%s,Directed,%s,,%s' % (flwr,target_id,str(edge_id_counter),user_similarity[int(target_id)][int(flwr)])
