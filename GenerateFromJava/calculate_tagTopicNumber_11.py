...
if __name__ == '__main__':
    ...
    user_interest_list=[]
    for line in open('/Users/makao/Yun/Workspace/lab/data/java_relation/num_user_interest.txt'):
        if line.strip() == '用户-兴趣':
            continue
    ...
    f1 = open('/Users/makao/Yun/Workspace/lab/data/java_relation/tag_topic_num_total.txt','w')
    f2 = open('/Users/makao/Yun/Workspace/lab/data/java_relation/not_interest_tag_number_total.txt','w')
    for item in result:
        print >> f1, '(%s, %s)' % (int(item[0]),int(item[1]))
    #打印非兴趣标签名，编号和其次数的对应关系
    for (key,value) in not_interest_tag_dict.items():
        print >> f2, '%s %s %s' % (value.split('#')[1].strip(), key, value.split('#')[0].strip())
    f1.close()
    f2.close()
